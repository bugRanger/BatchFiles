...
REM 3. {DB_PROVIDER} - сервер (имя/адрес).
REM 4. {DB_NAME} - база данных.
REM 5. {DB_USER} - пользователь.
REM 6. {DB_PASS} - пароль пользователя.
REM 7. {Attemp} - повторы выполнения при наличие ошибок (1 по умолчанию)
REM 8. {Silent} - тихий режим(0 - off, 1 - on. off по умолчанию).
REM 9. {LogFile} - файл для сбора результатов выполнения.
